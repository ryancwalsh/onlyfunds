...
contract Project is ERC20, Ownable {
    ...
    uint private _secondsPerPhase;
    uint public phases = 4;
    uint public totalFunding;
    bool public failed;
    bool public fundingPeriod = true;
    mapping(uint => bool) private _phasePayouts;
    ...
    function emergencyWithdraw() external {
        require(failed, "Project has not failed");
        require(balanceOf(msg.sender) > 0);
        uint percentage = balanceOf(msg.sender) * 1000 / _totalSupply;
        uint partialAmount = percentage * address(this).balance / 1000;
        _burn(msg.sender, partialAmount);
        payable(msg.sender).transfer(partialAmount);
    }

    function withdrawInvestment() public onlyOwner {
        require(_phasePayouts[currentPhase] == false);
        _phasePayouts[currentPhase] = true;
        uint amount = totalFunding / phases;
        payable(owner()).transfer(amount);
        emit Partialcashout(owner(), amount, block.timestamp);
    }

    function vote(bool personalVote) public {
        require(balanceOf(msg.sender) > 0);
        votes[msg.sender] = personalVote;
    }
